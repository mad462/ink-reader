...
static const uint8_t kFont5x7[59][5] = {
...
{
    if (buffer && length >= INK_EPD_BUFFER_SIZE) memset(buffer, white ? 0xff : 0x00, INK_EPD_BUFFER_SIZE);
}

void ink_epd_ui_set_pixel(uint8_t *buffer, size_t length, int x, int y, bool black)
{
    if (!buffer || length < INK_EPD_BUFFER_SIZE || x < 0 || y < 0 || x >= INK_EPD_WIDTH || y >= INK_EPD_HEIGHT) return;
    const size_t index = (size_t)y * (INK_EPD_WIDTH / 8) + (size_t)x / 8;
    const uint8_t mask = (uint8_t)(0x80u >> (x & 7));
    if (black) buffer[index] &= (uint8_t)~mask; else buffer[index] |= mask;
}

void ink_epd_ui_fill_rect(uint8_t *buffer, size_t length, int x, int y, int width, int height, bool black)
{
    for (int py = y; py < y + height; ++py) for (int px = x; px < x + width; ++px) ink_epd_ui_set_pixel(buffer, length, px, py, black);
}

void ink_epd_ui_draw_text(uint8_t *buffer, size_t length, int x, int y, int scale, const char *text, bool black)
{
    if (!text || scale < 1) return;
    for (; *text; ++text, x += 6 * scale) {
        const uint8_t *glyph = glyph_for(*text);
        for (int col = 0; col < 5; ++col) for (int row = 0; row < 7; ++row) {
            if (glyph[col] & (1u << row)) ink_epd_ui_fill_rect(buffer, length, x + col * scale, y + row * scale, scale, scale, black);
        }
    }
}
...
void ink_epd_ui_draw_launcher(uint8_t *buffer, size_t length, int selected)
{
    ...
    ink_epd_ui_draw_text(buffer, length, 120, 690, 2, "LEFT RIGHT  CONFIRM", true);
}

void ink_epd_ui_draw_status(uint8_t *buffer, size_t length, const char *title, const char *message)
{
    ink_epd_ui_clear(buffer, length, true);
    ink_epd_ui_draw_text(buffer, length, 40, 100, 4, title ? title : "INK", true);
    ink_epd_ui_fill_rect(buffer, length, 40, 180, 400, 4, true);
    ink_epd_ui_draw_text(buffer, length, 40, 260, 3, message ? message : "ERROR", true);
}
...
bool ink_epd_ui_self_test(void)
{
    ...
    for (size_t i = 0; i < sizeof(buffer); ++i) if (buffer[i] != 0xff) return true;
    return false;
}
