...
static bool parse_header(const uint8_t *header, size_t length, ink_font_info_t *font)
{
    if (!header || length < 32 || !font || memcmp(header, kMagic, sizeof(kMagic)) != 0) return false;
...
bool ink_fonts_probe_file(const char *path, ink_font_info_t *font)
{
    ...
    if (snprintf(font->path, sizeof(font->path), "%s", path) >= (int)sizeof(font->path)) {
        memset(font, 0, sizeof(*font));
        return false;
    }
    return true;
...
bool ink_fonts_load_default(ink_font_info_t *font)
{
    ...
    for (size_t i=0; i<sizeof(paths)/sizeof(paths[0]); ++i) if (ink_fonts_probe_file(paths[i], font)) return true;
    memset(font, 0, sizeof(*font));
    return false;
}
...
bool ink_fonts_self_test(void)
{
    ...
    uint8_t bad[32] = {0}; ink_font_info_t font = {0};
    return parse_header(good, sizeof(good), &font) && font.loaded && font.version == 4
        && !parse_header(bad, sizeof(bad), &font);
}
