...
static esp_err_t switch_to(const char *label)
{
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "set boot partition failed label=%s err=%s", label, esp_err_to_name(ret));
        return ret;
    }

    ESP_LOGI(TAG, "boot partition selected label=%s offset=0x%lx", label, (unsigned long)partition->address);
    esp_restart();
    return ESP_OK;
}
