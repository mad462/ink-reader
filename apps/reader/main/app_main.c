...
void app_main(void)
{
    ...
    if (sd_ret == ESP_OK && ink_fonts_load_default(&font)) {
        ESP_LOGI(TAG, "font loaded path=%s version=%u", font.path, (unsigned)font.version);
    } else {
    ...
    if (!book_ready) {
        ...
        ESP_LOGI(TAG, "book opened path=%s pages=%u", book.path, (unsigned)book.page_count);
    }
    ...
    while (true) {
        ...
        if (input_ret == ESP_OK && ink_input_poll(now_ms, &input) == ESP_OK) {
            ...
            if (book_ready && ink_input_was_pressed(&input, INK_BUTTON_LEFT)) page_changed = ink_reader_book_previous(&book);
            if (book_ready && ink_input_was_pressed(&input, INK_BUTTON_RIGHT)) page_changed = ink_reader_book_next(&book);
            if (page_changed) {
                if (ink_reader_book_load_current(&book, framebuffer, INK_EPD_BUFFER_SIZE)) {
                    esp_err_t ret = ink_hw_full_refresh(framebuffer, INK_EPD_BUFFER_SIZE);
                    if (ret != ESP_OK) ESP_LOGE(TAG, "page refresh failed err=%s", esp_err_to_name(ret));
                } else {
                    ESP_LOGE(TAG, "page load failed page=%u", (unsigned)book.current_page);
                }
            }
